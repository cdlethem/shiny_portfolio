...
shinyServer(function(input, output) {
  ...
  cleaned_dat <- reactive({
    response <-
      bind_rows(fromJSON("https://escwebtools.xyz/podio/fetch/pot"))
  ...
  filtered_dat <- reactive({
    ...
    if (input$pot_liaison == "All") {
      df_2 <- df_1
    } else
      df_2 <-
      filter_by_checkbox_input(df_1, "POT Call List", input$pot_liaison)
  ...
  output$pot_liaison_ui <- renderUI({
    tags$div(
      align = "left",
      class = "multicol",
      radioButtons(
        ...
        label = NULL,
        choices = c("All",
                       unique(cleaned_dat()$"POT Call List")),
        selected = "All"
      )
    )
  })
